  ...
  return (
    <div className="card info-panel">
      ...
      <div className="info-panel-section">
        ...
        {[
          { icon: '⬡', label: 'Project Overview', desc: 'Name, type, location, objective, stakeholders, timeline' },
          { icon: '◈', label: 'Key Requirements', desc: 'Categorised by Design, Workplace Strategy, Engineering, Delivery, Operations' },
          { icon: '?', label: 'Open Questions', desc: 'Missing or ambiguous information flagged for follow-up' },
          { icon: '⚑', label: 'Risks & Dependencies', desc: 'Identified risks with severity and supporting evidence' },
          { icon: '✦', label: 'Action Items', desc: 'Suggested next steps with owner and priority' },
          { icon: '◎', label: 'AI Opportunities', desc: '3 areas where AI could assist this project workflow' },
        ].map(item => (
          <div key={item.label} className="info-item" style={{ marginBottom: '0.625rem' }}>
            <span style={{ color: 'var(--color-accent)', fontSize: '0.75rem', width: 16, flexShrink: 0, fontWeight: 600 }}>
              {item.icon}
            </span>
            <span>
      ...
      <div className="info-panel-section">
        ...
        <div className="info-item">
          <span className="info-icon" style={{ fontSize: '0.75rem' }}>●</span>
